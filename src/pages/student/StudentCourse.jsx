...
<<<<<<< HEAD
        <div className="anim-fade-up" style={{ position:'relative', overflow:'hidden', display:'flex', flexDirection:'column', gap:'20px', marginBottom:'40px', padding:'30px', borderRadius:'24px', background: themeColor, boxShadow:`0 15px 40px -10px ${themeColor}80` }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', zIndex:1, flexWrap:'wrap', gap:'20px' }}>
            <div>
              <button onClick={()=>navigate('/student')} style={{ display:'inline-flex', alignItems:'center', gap:'6px', background:'rgba(255,255,255,0.2)', border:'1px solid rgba(255,255,255,0.4)', borderRadius:'10px', padding:'8px 14px', cursor:'pointer', color:'#fff', fontSize:'13px', fontWeight:700, marginBottom:'20px', transition:'.2s', backdropFilter:'blur(5px)' }} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.3)'} onMouseLeave={e=>e.currentTarget.style.background='rgba(255,255,255,0.2)'}>
                ← Panel Principal
              </button>
              ...
              return (
                <Card 
                  ...
                  style={{ 
                    ...
                    background: darkCyberpunkBg, 
                    transition:'all .3s', 
                    position:'relative', 
                    overflow:'hidden',
                    boxShadow: `inset 0 0 20px rgba(0,0,0,0.8)` 
                  }} 
                  onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-4px)'; e.currentTarget.style.boxShadow=`0 10px 25px ${activeColor}40, inset 0 0 20px rgba(0,0,0,0.8)`; e.currentTarget.style.borderColor = activeColor; }} 
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'20px', flexWrap:'wrap', zIndex:1, position:'relative' }}>
                    <div style={{ flex:1, minWidth:'250px' }}>
                      ...
                      <div style={{ color:C.textSub, fontSize:'13px', marginBottom:'16px', display:'flex', gap:'16px', alignItems:'center', flexWrap:'wrap' }}>
                        ...
                        {quizPct !== null && (
                          <span style={{ padding:'4px 10px', borderRadius:'8px', background:`${quizPct>=70?C.green:quizPct>=50?C.amber:C.red}20`, color:quizPct>=70?C.green:quizPct>=50?C.amber:C.red, fontWeight:800, border:`1px solid ${quizPct>=70?C.green:quizPct>=50?C.amber:C.red}50` }}>
                            🎯 Score: {quizPct}%
                          </span>
                        )}
                      </div>
